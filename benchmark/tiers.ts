...
async function main() {
  ...
  try { await fetch(`${API}/warmup`, { method: "POST", signal: AbortSignal.timeout(30_000) }); } catch {}
  await new Promise((r) => setTimeout(r, 1000));
  console.log("");

  interface Result { label: string; p50: number; p95: number; p99: number; avg: number; ok: number }
  ...
  for (const tier of TIERS) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      try {
        ...
        if (apiKey) { headers["X-API-Key"] = apiKey; headers["Referer"] = "http://localhost:8000"; }
        const res = await fetch(`${API}/suggest?q=${encodeURIComponent(q)}&limit=10&no_cache=1`, { signal: AbortSignal.timeout(10_000), headers });
