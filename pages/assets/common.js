...
function showUpdateModal(currentVersion, latestVersion, downloadUrl) {
  ...
  modal.innerHTML =
    ...
    '<a href="' +
    escAttr(downloadUrl || "#") +
    '" target="_blank" rel="noopener" class="btn btn-sm btn-green">Download from data.gov.au &nearr;</a>' +
...
// biome-ignore lint/correctness/noUnusedVariables: used by suggest.js et al.
...
document.addEventListener("key-expired", () => {
  const suggestStatus = document.getElementById("suggestApiKeyStatus");
  if (suggestStatus) {
    const key = getApiKey();
    suggestStatus.textContent = `✗ Expired: ${key ? maskKey(key) : ""}`;
  }
});
...
async function fetchTabContent(tabName) {
  ...
  try {
    ...
    if (!resp.ok)
      return `<p class="text-red">Failed to load ${tabName} tab (HTTP ${resp.status}).</p>`;
  ...
  document.querySelectorAll(".tab").forEach((tab) => {
    tab.addEventListener("click", async () => {
      ...
      if (!panel.dataset.loaded) {
        panel.innerHTML =
          '<p style="padding:12px;color:var(--muted);font-size:var(--fs-sm)">Loading…</p>';
    ...
    if (panel && !panel.dataset.loaded) {
      fetchTabContent(activeTab.dataset.tab).then((content) => {
        ...
        document.dispatchEvent(
          new CustomEvent("tab-loaded", { detail: { tab: activeTab.dataset.tab } }),
        );
      });
    }
  }
