...
const ERROR_MESSAGES = {
  ...
  CANNOT_SELF_REVOKE:
    "You cannot revoke your own key while other active keys exist. Revoke them first, or use DNS recovery to revoke all.",
...
function initKeyGeneration() {
  ...
  async function generateKey() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const tableRows = data.keys
          .map((k, idx) => {
            const _keyPrefix = k.key.slice(8, 16);
            return (
              "<tr>" +
              '<td style="padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted);font-size:0.7rem;width:2em">' +
              (idx + 1) +
              "</td>" +
              '<td style="padding:6px 8px;border-bottom:1px solid var(--border);font-family:var(--mono);font-size:var(--fs-xs);word-break:break-all">' +
              esc(k.key) +
              "</td>" +
              '<td style="padding:6px 8px;border-bottom:1px solid var(--border);text-align:right;white-space:nowrap">' +
              "<button onclick=\"copyKey('" +
              esc(k.key) +
              '\',this)" style="background:transparent;border:1px solid var(--border);color:var(--base00);padding:3px 6px;border-radius:3px;cursor:pointer;vertical-align:middle;line-height:1" title="Copy key">' +
        ...
        showResult(
          '<div style="color:var(--green);font-weight:600;margin-bottom:6px">' +
            esc(String(data.generated_count)) +
            " key(s) generated for <strong>" +
            esc(data.domain) +
            "</strong></div>" +
            '<div style="font-size:var(--fs-xs);color:var(--muted);margin-bottom:8px">' +
            esc(String(data.total_for_domain)) +
            " of " +
            esc(String(data.max_allowed)) +
            " max keys used for this domain.</div>" +
            ...
            '<div style="font-size:var(--fs-xs);color:var(--muted);margin-bottom:8px">Add this TXT record to <strong>' +
            esc(data.domain) +
            "</strong> DNS, then click Verify:</div>" +
            ...
            '<span style="flex:1;font-family:var(--mono);font-size:var(--fs-xs);word-break:break-all;color:var(--cyan)">gnaf-verify=' +
            esc(verifyToken) +
            "</span>" +
            "<button onclick=\"copyKey('gnaf-verify=" +
            esc(verifyToken) +
            '\',this)" style="background:transparent;border:1px solid var(--border);color:var(--base00);padding:3px 6px;border-radius:3px;cursor:pointer;flex-shrink:0;line-height:1" title="Copy TXT record">' +
            ...
            esc(firstPrefix) +
            '\')" style="background:var(--blue);color:#fff;border:none;border-radius:4px;padding:6px 16px;cursor:pointer;font-size:var(--fs-sm)">Verify Now</button>' +
        ...
        showResult(
          `<div style="color:var(--orange);font-size:var(--fs-sm)">${esc(handleApiError(data, "Failed"))}</div>`,
        );
      }
    } catch (e) {
      showResult(
        `<div style="color:var(--orange);font-size:var(--fs-sm)">${esc(e?.message || e || "Network error")}</div>`,
      );
    }
  ...
  domainInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !submitBtn.disabled) generateKey();
  });
}
...
function initKeyManagement() {
  ...
  btn.addEventListener("click", async () => {
    ...
    if (!domain || !apiKey) {
      errorEl.textContent = "Enter both domain and API key.";
      return;
    }
  ...
  function renderMgmtTable(domain, keys) {
    ...
    const fmt = (n) =>
      n
        ? new Date(n).toLocaleDateString("en-AU", {
            day: "numeric",
            month: "short",
            year: "numeric",
          })
    ...
    let html =
      '<div style="color:var(--green);font-weight:600;margin-bottom:8px">✓ Verified! Keys for <strong>' +
      esc2(domain) +
      "</strong>:</div>" +
      ...
      const sc =
        k.status === "active"
          ...
          : k.status === "pending"
            ? "var(--orange)"
            : "var(--muted)";
      const expiresHtml = `<td style="padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted)">${k.expires_at ? fmtExpiry(k.expires_at) : "-"}</td>`;
      html +=
        "<tr>" +
        '<td style="padding:6px 8px;border-bottom:1px solid var(--border);font-family:var(--mono)">' +
        esc2(k.prefix) +
        "</td>" +
        ...
        "</td>" +
        expiresHtml +
        '<td style="padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted)">' +
        fmt(k.created_at) +
        "</td>" +
        '<td style="padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted)">' +
        esc2(String(k.request_count)) +
        "</td>" +
        (k.status !== "revoked"
  ...
  window.mgmtRevoke = async (prefix) => {
    if (!prefix || !confirm(`Revoke key prefix "${prefix}"?`)) return;
    if (!_mgmtKey) {
      alert("Session expired. Re-enter your API key.");
      return;
    }
    try {
...
function initKeyRecovery() {
  ...
  btn.addEventListener("click", async () => {
    const domain = domainInput.value.trim();
    if (!domain) {
      stepEl.textContent = "Enter a domain.";
      return;
    }
    ...
    stepEl.innerHTML =
      '<span style="color:var(--muted)">⏳ Requesting verification token...</span>';
    ...
    try {
      ...
      stepEl.innerHTML =
        ...
        '<div style="font-weight:600;font-size:var(--fs-sm);margin-bottom:4px">📝 Add this TXT record to <strong>' +
        esc(data.domain) +
        "</strong> DNS:</div>" +
        '<div style="display:flex;align-items:center;gap:6px;background:var(--bg);border:1px solid var(--border);border-radius:4px;padding:8px 10px;margin-bottom:8px">' +
        '<span style="flex:1;font-family:var(--mono);font-size:var(--fs-xs);word-break:break-all;color:var(--cyan)">gnaf-mgmt=' +
        esc(data.verification_token) +
        "</span>" +
        "<button onclick=\"copyKey('gnaf-mgmt=" +
        esc(data.verification_token) +
        '\',this)" style="background:transparent;border:1px solid var(--border);color:var(--base00);padding:3px 6px;border-radius:3px;cursor:pointer;flex-shrink:0;line-height:1" title="Copy TXT record">' +
        '<svg viewBox="0 0 16 16" fill="currentColor" width="14" height="14"><path d="M5 4.5v-2A1.5 1.5 0 016.5 1h5A1.5 1.5 0 0113 2.5v7a1.5 1.5 0 01-1.5 1.5h-2"/><path d="M3 5.5A1.5 1.5 0 014.5 4h5A1.5 1.5 0 0111 5.5v7A1.5 1.5 0 019.5 14h-5A1.5 1.5 0 013 12.5z"/></svg></button></div>' +
  ...
  window.recoverVerify = async () => {
    ...
    try {
      ...
      if (data.status === "verified") {
        ...
        let html =
          '<div style="color:var(--green);font-weight:600;margin-bottom:8px">✓ Domain verified — review keys below:</div>' +
          '<table style="width:100%;border-collapse:collapse;font-size:var(--fs-xs)"><thead><tr>' +
          '<th style="text-align:left;padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted)">Prefix</th>' +
          '<th style="text-align:left;padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted)">Status</th>' +
          '<th style="text-align:left;padding:6px 8px;border-bottom:1px solid var(--border);color:var(--muted)">Expires</th>' +
        ...
        html += "</tbody></table>";
        html +=
          '<div style="font-size:0.65rem;color:var(--muted);margin-top:6px">Keys auto-extend on use — expires if unused for 90 days.</div>';

        // Bulk-revoke button
        html +=
          '<div style="margin-top:12px;padding:10px;background:var(--surface-2);border:1px solid var(--border);border-radius:4px">' +
        ...
        // Warning banner
        html +=
          '<div style="background:var(--surface-2);border-left:3px solid var(--red);padding:8px 10px;margin-top:8px;font-size:var(--fs-xs)">' +
        ...
        statusEl.innerHTML =
          '<span style="color:var(--orange)">⏳ TXT record not found. It can take a few minutes to propagate.</span>';
      }
    } catch (e) {
  ...
  window.recoverBulkRevoke = async () => {
    const statusEl = document.getElementById("ak-bulk-revoke-status");
    if (!statusEl) return;
    if (!currentToken) {
      statusEl.textContent = "Session expired. Start recovery again.";
      return;
    }

    if (
      !confirm(
        `⚠ Are you sure? This will revoke ALL active API keys for ${currentDomain}. This action is irreversible.`,
      )
    )
      return;
...
window.revokeKey = async (prefix) => {
  ...
  const apiKey = keyFromUrl || getApiKey();
  if (!apiKey) {
    statusEl.innerHTML =
      '<span style="color:var(--orange)">✗ No API key available. Set one in the header bar.</span>';
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      statusEl.innerHTML =
        '<span style="color:var(--green)">✓ Key revoked. You can now generate a new one.</span>';
    } else if (data.code === "CANNOT_SELF_REVOKE") {
...
window.verifyKey = async (prefix) => {
  ...
  try {
    ...
    if (data.status === "verified" || data.status === "active") {
      ...
      statusEl.innerHTML =
        '<span style="color:var(--orange)">⏳ TXT record not found yet. It can take a few minutes to propagate.</span>';
    } else if (data.status === "dns_error") {
      statusEl.innerHTML =
        '<span style="color:var(--orange)">⚠ Could not query DNS. Check the domain name.</span>';
    } else {
...
function fmtExpiry(dateStr) {
  ...
  if (daysLeft < 0)
    return `<span style="color:var(--red)" title="Key expired">${formatted} (expired)</span>`;
  if (daysLeft <= 30)
    return `<span style="color:var(--orange)" title="Expires soon — keep using the key to auto-extend">${formatted}</span>`;
  return formatted;
}
